...
function getUserCompletedModules($conn, $userId) {
    $completedModules = ['video' => [], 'research' => []];
...
$allVideoModules = getAllActiveVideoModules($conn);
...
<head>
    ...
    <title>GoRako - Semua Video</title>
    <meta name="description" content="Jelajahi semua modul video edukasi tentang pengelolaan sampah dan lingkungan di GoRako.">
    <meta name="keywords" content="GoRako, modul video, video edukasi, daur ulang, lingkungan, belajar">
    ...
    <style>
        ...
        * {
        ...
        .module-card-image { /* Used for video thumbnails */
            width: 100%;
            height: 200px;
            object-fit: cover;
...
<body>
    ...
    <main>
        ...
        <div class="container-modules">
            <section id="all-videos-section">
                <div class="section-header">
                    <h2>Semua Modul Video</h2>
                    <p>Temukan semua video edukasi inspiratif tentang pengelolaan sampah, daur ulang, dan isu lingkungan lainnya.</p>
                </div>
                <div class="modules-grid">
                    <?php if (!empty($allVideoModules)): ?>
                        <?php foreach ($allVideoModules as $index => $module): // Added $index for staggered animation
                            $isCompleted = in_array($module['id'], $userCompletedModules['video']);
                            // Truncate title to ensure it's not too long
                            $displayTitle = strlen($module['title']) > 70 ? substr($module['title'], 0, 67) . '...' : $module['title'];
                        ?>
                            <div class="module-card animate-on-scroll" data-module-id="<?= htmlspecialchars($module['id']); ?>" data-module-type="video" style="transition-delay: <?= $index * 0.1 ?>s;">
                                <?php if (!empty($module['thumbnail_url'])): ?>
                                    <img src="<?= htmlspecialchars($module['thumbnail_url']); ?>" alt="<?= htmlspecialchars($module['title']); ?> Thumbnail" class="module-card-image" onerror="this.onerror=null;this.src='https://placehold.co/600x400/cccccc/333333?text=Video+Thumbnail';">
                                <?php else: ?>
                                    <div class="module-card-image placeholder"><i class="fas fa-video"></i></div>
                                <?php endif; ?>
                                <div class="module-content">
                                    <h3 class="module-title"><?= htmlspecialchars($displayTitle); ?></h3>
                                    <div class="module-info">
                                        <span><i class="far fa-clock"></i> <?= htmlspecialchars($module['duration_minutes']); ?> menit</span>
                                        <span><i class="fas fa-coins"></i> <?= htmlspecialchars($module['points_reward']); ?> Poin</span>
                                    </div>
                                    <div class="module-actions">
                                        <a href="<?= htmlspecialchars($module['video_url']); ?>" target="_blank" class="module-action-button secondary">
                    ...
                    <?php else: ?>
                        <div class="no-modules-placeholder">
                            <p>Tidak ada modul video aktif saat ini.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </section>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- Modal Functions (for Research Text Content - included for consistency, not directly used here) ---
                ...
                button.addEventListener('click', async function() {
                    ...
                    try {
                        const response = await fetch(endpoint, {
                            ...
                            body: `module_id=${moduleId}&action=complete_${moduleType}`
                        });

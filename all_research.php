...
function getUserCompletedModules($conn, $userId) {
    $completedModules = ['video' => [], 'research' => []]; // Initialize with both types
    $stmt = $conn->prepare("SELECT module_type, module_id FROM user_module_progress WHERE user_id = ? AND is_completed = 1");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
    $result = $stmt->get_result();
    while($row = $result->fetch_assoc()) {
        $completedModules[$row['module_type']][] = $row['module_id'];
    }
    $stmt->close();
...
$allResearchModules = getAllActiveResearchModules($conn);
$userCompletedModules = getUserCompletedModules($conn, $loggedInUserId);
...
<head>
    ...
    <title>GoRako - Semua Modul Riset</title>
    <meta name="description" content="Jelajahi semua modul riset dan bacaan tentang pengelolaan sampah dan lingkungan di GoRako.">
    <meta name="keywords" content="GoRako, modul riset, bacaan lingkungan, daur ulang, artikel edukasi">
    ...
    <style>
        ...
        * {
        ...
        .module-card { /* Used for all_videos, all_research */
        ...
        .module-card-image { /* Used for video thumbnails, placeholder for research */
            width: 100%;
            height: 200px;
            object-fit: cover;
        ...
        .module-action-button.secondary:hover {
        ...
        /* Modal Styles (for research text content) */
        .modal-overlay {
            position: fixed;
            top: 0;
            left: 0;
...
<body>
    ...
    <main>
        ...
        <div class="container-modules">
            <section id="all-research-section">
                <div class="section-header">
                    <h2>Semua Modul Riset & Bacaan</h2>
                    <p>Perdalam pemahaman Anda dengan semua artikel riset dan bacaan mendalam tentang topik lingkungan dan daur ulang.</p>
                </div>
                <div class="modules-grid">
                    <?php if (!empty($allResearchModules)): ?>
                        <?php foreach ($allResearchModules as $index => $module): // Added $index for staggered animation
                            $isCompleted = in_array($module['id'], $userCompletedModules['research']);
                            // Truncate title to ensure it's not too long
                            $displayTitle = strlen($module['title']) > 70 ? substr($module['title'], 0, 67) . '...' : $module['title'];
                        ?>
                            <div class="module-card animate-on-scroll" data-module-id="<?= htmlspecialchars($module['id']); ?>" data-module-type="research" style="transition-delay: <?= $index * 0.1 ?>s;">
                                <div class="module-card-image placeholder"><i class="fas fa-book-open"></i></div>
                                <div class="module-content">
                                    <h3 class="module-title"><?= htmlspecialchars($displayTitle); ?></h3>
                                    <div class="module-info">
                                        <span><i class="far fa-hourglass-half"></i> Estimasi <?= htmlspecialchars($module['estimated_minutes']); ?> menit</span>
                                        <span><i class="fas fa-coins"></i> <?= htmlspecialchars($module['points_reward']); ?> Poin</span>
                                    </div>
                                    <div class="module-actions">
                                        <?php if ($module['content_type'] === 'text'): ?>
                                            <button class="module-action-button secondary view-text-content"
                                                    data-title="<?= htmlspecialchars($module['title']); ?>"
                                                    data-content="<?= htmlspecialchars($module['text_content']); ?>">
                                                <i class="fas fa-glasses"></i> Baca Konten
                                            </button>
                                        <?php else: ?>
                                            <a href="<?= htmlspecialchars($module['content_url']); ?>" target="_blank" class="module-action-button secondary">
                                                <i class="fas fa-external-link-alt"></i> Lihat Konten
                                            </a>
                                        <?php endif; ?>
                                        <button class="module-action-button primary mark-complete-button"
                                                data-module-id="<?= htmlspecialchars($module['id']); ?>"
                                                data-module-type="research"
                                                <?= $isCompleted ? 'disabled' : ''; ?>>
                                            <i class="fas fa-check-circle"></i> <?= $isCompleted ? 'Selesai' : 'Tandai Selesai'; ?>
                                        </button>
                                    </div>
                                </div>
                                <?php if ($isCompleted): ?>
                                    <span class="module-completed-badge">Telah Selesai</span>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="no-modules-placeholder">
                            <p>Tidak ada modul riset atau bacaan aktif saat ini.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </section>
    ...
    <div class="modal-overlay" id="researchModal">
        <div class="modal-content">
            <button class="modal-close-button" onclick="closeModal('researchModal')">&times;</button>
            <h3 id="modalTitle"></h3>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Select all module-card elements for animation and tilt
            ...
            // --- Modal Functions (for Research Text Content) ---
            ...
            function openModal(modalId, title = '', content = '') {
                ...
                if (modalId === 'researchModal') {
                    modalTitle.textContent = title;
                    modalContent.innerHTML = `<p>${content}</p>`; // Wrap content in a paragraph
                }
            ...
            document.querySelectorAll('.view-text-content').forEach(button => {
                button.addEventListener('click', () => {
                    const title = button.dataset.title;
                    const content = button.dataset.content;
                    openModal('researchModal', title, content);
                });
            });
            ...
            document.querySelectorAll('.mark-complete-button').forEach(button => {
                button.addEventListener('click', async function() {
                    ...
                    const moduleType = this.dataset.moduleType; // 'video' or 'research'

                    const confirmAction = confirm(`Apakah Anda yakin telah menyelesaikan modul ${moduleType} ini? Anda akan mendapatkan poin!`);

                    if (!confirmAction) {
                        return;
                    }

                    this.disabled = true; // Disable button immediately to prevent double clicks
                    ...
                    let endpoint = '';
                    if (moduleType === 'video') {
                        endpoint = 'module_video_view.php';
                    } else if (moduleType === 'research') {
                        endpoint = 'module_reading_view.php';
                    } else {
                        showToast('Tipe modul tidak valid.', 'error');
                        this.disabled = false;
                        this.textContent = 'Tandai Selesai';
                    ...
                    try {
                        const response = await fetch(endpoint, {
                            ...
                            body: `module_id=${moduleId}&action=complete_${moduleType}` // 'complete_video' or 'complete_reading'
                        });
                        ...
                        if (result.status === 'success') {
                            ...
                            // Update UI: change button to 'Selesai' and add 'Telah Selesai' badge
                            this.textContent = 'Selesai';
                            this.disabled = true;
                            const moduleCard = this.closest('.module-card');
                            if (moduleCard) {
                                let badge = moduleCard.querySelector('.module-completed-badge');
                                if (!badge) {
                                    badge = document.createElement('span');
                                    badge.classList.add('module-completed-badge');
                                    moduleCard.appendChild(badge);
                                }
                                badge.textContent = 'Telah Selesai';
                                badge.style.backgroundColor = userAppearanceSettings.accent_color; // Apply accent color
                            }
                        } else if (result.status === 'info') {
                            showToast(result.message, 'info');
                            // Update UI if already completed (e.g., if user refreshed page)
                            this.textContent = 'Selesai';
                            this.disabled = true;
                            const moduleCard = this.closest('.module-card');
                             if (moduleCard) {
                                ...
                                badge.textContent = 'Telah Selesai';
                                badge.style.backgroundColor = userAppearanceSettings.accent_color;
                            }
                        } else {
                            showToast(result.message || 'Terjadi kesalahan saat menandai modul.', 'error');
                            this.disabled = false;
                            this.textContent = 'Tandai Selesai';
                        }
                    } catch (error) {
